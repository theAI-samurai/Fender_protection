...
def main_program(cam_, cam_url):
    ...
    while True:
        ...
        global active_cams
        global OPENCV_PLAYER_OBJECT
        global result

        print(cam_, active_cams)

        if cam_ not in active_cams:                                     # if camID is inactive initialize the camera
            is_active = active_stream_initialize_opencv_v2(cam_, cam_url)     # checks if frame available
            if is_active:                                               # if True
                restart_status = False
                start_timer = 0
                active_cams.append(cam_)

                opencv_obj = cv2.VideoCapture(cam_url, cv2.CAP_FFMPEG)
                OPENCV_PLAYER_OBJECT.update({cam_: opencv_obj})
            else:
                gc.collect()
            ...
            if not restart_status:
                ...
                alive, read = read_frames_using_opencv(p_obj=opencv_obj, delay_time=1, path=frame_path, camid=cam_)

                fgmask = None
                contours = []
                draw = 0

                if alive is True and read is True:                       # OPENCV link is receiving
                ...
                # if Frame is not Read
                    ...
                    if start_timer != 0 and time.time() - start_timer > 15:
                        ...
                        opencv_obj.release()
                        OPENCV_PLAYER_OBJECT[cam_].release()
                        del OPENCV_PLAYER_OBJECT[cam_]         # del VLC object of cameraID
                        del opencv_obj
                        gc.collect()
